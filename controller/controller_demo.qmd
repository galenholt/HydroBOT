...
This *runs* but doesn't *render* currently- `controller_functions` can't be found during render unless it's a package.
```{python}
import os
import git
import time
...
scenario_dir = os.path.join(repo, 'data', 'testsmall')

output_dir = os.path.join(repo, 'data', 'testsmall')
```
...
mod_format = 'IQQM - NSW 10,000 years'
```

# Below here turns into wrappers
...
ewr_summary = controller_functions.run_save_ewrs(everyhydro, outpath, format = mod_format, allowance = allowance, climate = climate, outputType = outputType, datesuffix = False, returnType = 'summary')
```

...
tarpath <- system.file(
  'data-raw/causal_networks/unknown/Env_objectives_Target_species_nodes.csv',
  package = 'HydroBOT'
)

qcs <- c(
  system.file(
    'data-raw/causal_networks/unknown/Target_species_per_PU_Murray.csv',
    package = 'HydroBOT'
  ),
...
test_that("produces desired format for obj-target", {
  ...
  expect_snapshot_value(as.list(unique(obj2target$env_obj[
    !is.na(obj2target$env_obj)
  ])))
  expect_snapshot_value(as.list(unique(obj2target$Specific_goal[
    !is.na(obj2target$Specific_goal)
  ])))
  expect_snapshot_value(as.list(unique(obj2target$Objective[
    !is.na(obj2target$Objective)
  ])))
  expect_snapshot_value(as.list(unique(obj2target$Target[
    !is.na(obj2target$Target)
  ])))
})
...
test_that("above the planning unit scale", {
  ...
  expect_equal(
    names(obj2target),
    c('LTWPShortName', 'env_obj', 'Specific_goal', 'Objective', 'Target')
  )
  # test the values are there
  expect_snapshot_value(as.list(unique(obj2target$env_obj[
    !is.na(obj2target$env_obj)

test_that("long data referencing works", {
  ...
  baselong <- baseline_compare(hydlong,
    compare_col = "scenario", comp_fun = difference,
...
test_that("auto-grouper works", {
  ...
  expect_warning(baselong <- baseline_compare(hydlong,
    compare_col = "scenario", comp_fun = difference,
...
test_that("multiple values columns works", {
  hydwide <- read_hydro(
    hydropath = system.file("extdata/testsmall/hydrographs",
      package = "werptoolkitr"
    ),
    long = FALSE
  )
  expect_warning(basewide <- baseline_compare(hydwide,
    compare_col = "scenario", comp_fun = difference,
...
test_that("scalar refs works", {
  ...
  basescalar <- baseline_compare(hydlong,
    compare_col = "scenario", comp_fun = difference,
    base_lev = 0, values_col = "flow", group_cols = c("Date", "gauge")
  )

  expect_warning(basescalarVals <- baseline_compare(hydwide,
    compare_col = "scenario", comp_fun = difference,
...
test_that("relative comparison works", {
  ...
  baselong <- baseline_compare(hydlong,
    compare_col = "scenario", comp_fun = relative,
    base_lev = "base_base", values_col = "flow", group_cols = c("Date", "gauge")
  )
  # names are right
...
test_that("relative comparison works with add_eps", {
  ...
  baselong <- baseline_compare(hydlong,
    compare_col = "scenario", comp_fun = relative, add_eps = 1,
    base_lev = "base_base", values_col = "flow", group_cols = c("Date", "gauge")
  )
  # names are right
  expect_equal(names(baselong), c("scenario", "Date", "gauge", "flow", "ref_flow", "relative_flow"))

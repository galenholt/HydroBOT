...
test_that("ewr-obj works, nongeom", {
  ...
  agged <- multi_aggregate(
    non_spatial_ewrout,
    aggsequence = list(c("ewr_code", "ewr_code_main")),
    groupers = "scenario",
    aggCols = "ewr_achieved",
    funsequence = list("mean"),
    causal_edges = make_edges(causal_ewr, list(c("ewr_code", "ewr_code_main"))),
    auto_ewr_PU = TRUE
  )
  expect_equal(
    names(agged),
...
test_that("gauge to poly works", {
  ...
  expect_equal(
    nrow(spataggkeep),
    nrow(sdl_units) * length(unique(ewr_to_agg_timemean$scenario))
  )
...
test_that("poly to poly works", {
  ...
  namestring <- c(
    "scenario",
    "polyID",
    "cewo_valleys_mean_sdl_units_mean_ewr_achieved",
    "ValleyName",
    "ValleyID",
    "ValleyCode",
    "geometry"
...
test_that("bare functions", {
  ...
  expect_equal(sort(names(spatagg)), sort(namestring))
  expect_s3_class(spatagg, "sf")
  expect_equal(nrow(spatagg), length(unique(ewr_to_agg_timemean$scenario)) * 2)
})
...
test_that("multiple functions", {
  ...
  expect_equal(
    nrow(spatagg_c),
    length(unique(ewr_to_agg_timemean$scenario)) * 2
  )

  # bare- naming fails
  expect_warning(
    spatagg_b <- multi_aggregate(
      ewr_to_agg_timemean,
      aggsequence = list(sdl_units = sdl_units),
      groupers = "scenario",
      aggCols = "ewr_achieved",
...
test_that("multi-step spatial works", {
  ...
  expect_equal(sort(names(spatagg)), sort(namestring))
  expect_s3_class(spatagg, "sf")
  expect_equal(nrow(spatagg), length(unique(spatagg$scenario)))
  ...
  expect_equal(
    nrow(spataggkeep),
    nrow(basin) * length(unique(ewr_to_agg_timemean$scenario))
  )
  ...
  g2sdl_plot <- ggplot2::ggplot() +
    ggplot2::geom_sf(
      data = spatagg,
      ggplot2::aes(
        fill = basin_ArithmeticMean_cewo_valleys_ArithmeticMean_sdl_units_ArithmeticMean_ewr_achieved
      )
    ) +
    ggplot2::geom_sf(data = ewr_to_agg_timemean) +
    ggplot2::facet_wrap(~scenario) +
...
test_that("multi-step theme and spatial works", {
  ...
  expect_equal(
    nrow(spatagg),
    length(unique(ewr_to_agg_timemean$scenario)) *
      length(unique(causal_ewr$theme_to_ewr_code_main$theme[
        !is.na(causal_ewr$theme_to_ewr_code_main$theme)
      ]))
  )
...
test_that("backstepping along theme axis throws informative error", {
  ...
  aggseq <- list(
    ...
    theme = c("objective_text", "theme"),
    test1 = c("theme", "testy"),
    test2 = c("eco_objective", "more_testy")
  )
...
test_that("saving the list of steps and appropriate persistence of PU grouping", {
  ...
  expect_true(
    "planning_unit_name" %in%
      names(spatagg$agg_input) &
      "planning_unit_name" %in% names(spatagg$ewr_code) &
      "planning_unit_name" %in% names(spatagg$eco_objective)
  )

  expect_true(
    !"planning_unit_name" %in% names(spatagg$sdl_units) &
      !"planning_unit_name" %in% names(spatagg$objective_text) &
  ...
  expect_true(
    "gauge" %in%
      ...
      "gauge" %in% names(spatagg$eco_objective)
  )

  expect_true(
    !"gauge" %in% names(spatagg$sdl_units) &
      !"gauge" %in% names(spatagg$objective_text) &
  ...
  expect_equal(nrow(spatagg$theme), 93)
  expect_equal(sum(is.na(spatagg$mdb)), 0)
...
test_that("nonspatial joins of spatial data", {
  ...
  # Find some NA- delete
  naexample <- spatagg$ewr_code_main |>
    dplyr::filter(scenario == 'base' & planning_unit_name == 'Muggabah Creek')
  naobj <- spatagg$eco_objective |>
    dplyr::filter(scenario == 'base' & planning_unit_name == 'Muggabah Creek')

  relevantewrs <- causal_ewr$ewr_code_to_ewr_code_main |>
    dplyr::filter(
      ewr_code_main %in%
        naexample$ewr_code_main &
        planning_unit_name %in% naexample$planning_unit_name
    )
  ...
  expect_equal(nrow(spatagg$ewr_code_main), nrow(spatagg$planning_units))
  ...
  puinpu <- spatagg$planning_units |>
    dplyr::summarise(
      ewr_achieved = mean(ewr_achieved, na.rm = T),
  ...
  g2puplot <- ggplot2::ggplot() +
    ggplot2::geom_sf(
      data = dplyr::filter(
        sdl_units,
        SWSDLName %in% unique(spatagg$sdl_units$SWSDLName)
      )
    ) +
    ggplot2::geom_sf(
      data = puinpu,
    ...
    ggplot2::geom_sf(
      data = ewr_to_agg_timemean |> dplyr::select(geometry) |> dplyr::distinct()
    ) +
    ggplot2::theme(legend.position = "bottom")
  ...
  expect_equal(
    unique(spatagg$sdl_units$SWSDLName),
    c("Murrumbidgee", "Lower Darling", 'Lachlan', 'Macquarie-Castlereagh')
  )
  ...
  aggseqc <- list(
    ...
    planning_units = planning_units,
    eco_objective = c("ewr_code_main", "eco_objective"),
    sdl_units = dplyr::filter(
      sdl_units,
      SWSDLName %in% c('Lachlan', 'Murrumbidgee', 'Macquarie-Castlereagh')
    ),
    objective_text = c("eco_objective", "objective_text"),
    catchment = cewo_valleys,
    theme = c("objective_text", "theme"),
  ...
  expect_equal(
    unique(spataggc$sdl_units$SWSDLName),
    c("Murrumbidgee", 'Lachlan', 'Macquarie-Castlereagh')
  )

  # check it works for group_until explicitly
  spatagg_g <- multi_aggregate(
    ewr_to_agg_timemean,
    aggsequence = aggseqc,
    groupers = "scenario",
    aggCols = "ewr_achieved",
  ...
  sortc <- spataggc$mdb |>
    sf::st_drop_geometry() |>
    dplyr::arrange(scenario, theme, polyID, ewr_achieved)
...
test_that("nonspatial joins of spatial data at sdl unit", {
  ...
  fakeewrs <- ewr_to_agg_timemean |>
    dplyr::mutate(
      SWSDLName = dplyr::case_when(
        SWSDLName == 'Lachlan' ~ 'Broken',
  ...
  expect_warning(
    spatagg_wrongsdl_spatial <- fakeewrs |>
      multi_aggregate(
        aggsequence = aggseq,
        groupers = "scenario",
        aggCols = "ewr_achieved",
  ...
  expect_equal(
    unique(spatagg_wrongsdl$sdl_units$SWSDLName),
    c('Goulburn', 'Broken')
  )
  # real names with spatial starting with fake names
  expect_equal(
    unique(spatagg_wrongsdl_spatial$sdl_units$SWSDLName),
    c('Lachlan', 'Macquarie-Castlereagh')
  )
  ...
  expect_true(
    'planning_unit_name' %in% names(spatagg_wrongsdl_spatial$ewr_code)
  )
})
...
test_that("single functions at each step, called in different ways", {
  ...
  expect_equal(nrow(spatagg_c$sdl_units), 165)
  ...
  expect_equal(nrow(spatagg_b$sdl_units), 165)
  ...
  expect_equal(nrow(spatagg_l$sdl_units), 165)
...
test_that("multiple functions at each step", {
  ...
  funseq_l <- list(
    ...
    list(
      ArithmeticMean = ~ ArithmeticMean(.),
      CompensatingFactor = ~ CompensatingFactor(.)
    )
  )
...
test_that("mixed functions between steps", {
  ...
  expect_type(spatagg, "list")
  expect_s3_class(spatagg[[length(spatagg)]], "sf")
  expect_equal(nrow(spatagg$sdl_units), 165)
...
test_that("group_until works", {
  ...
  expect_equal(nrow(spatagg$ewr_code), 276)
  ...
  expect_true("scenario" %in% names(spatagg$mdb))
...
test_that("Sequencing edge cases", {
  ...
  # warning because sdls should be done pseudo-spatially, but here we want to test spatial
  expect_warning(
    spatagg_s_th_t <- multi_aggregate(
      ewr_to_agg,
      causal_edges = causal_ewr,
      aggsequence = list(
        sdl_units = sdl_units,
  ...
  spatagg_s_t_th <- multi_aggregate(
    ewr_to_agg,
    causal_edges = causal_ewr,
    aggsequence = list(
      sdl_units = sdl_units,
  ...
  spatagg_th_s_t <- multi_aggregate(
    ewr_to_agg,
    causal_edges = causal_ewr,
    aggsequence = list(
      ewr_code_main = c('ewr_code', 'ewr_code_main'),
  ...
  spatagg_th_t_s <- multi_aggregate(
    ewr_to_agg,
    causal_edges = causal_ewr,
    aggsequence = list(
      ewr_code_main = c('ewr_code', 'ewr_code_main'),
  ...
  spatagg_t_s_th <- multi_aggregate(
    ewr_to_agg,
    causal_edges = causal_ewr,
    aggsequence = list(
      yrs = '2 years',
  ...
  spatagg_t_th_s <- multi_aggregate(
    ewr_to_agg,
    causal_edges = causal_ewr,
    aggsequence = list(
      yrs = '2 years',
...
test_that("Temporal", {
  ...
  spatagg_temp <- multi_aggregate(
    ewr_to_agg,
    causal_edges = causal_ewr,
    aggsequence = list(all_time = 'all_time'),
    groupers = c("scenario", 'ewr_code', 'ewr_code_main'),
    aggCols = "ewr_achieved",
    saveintermediate = TRUE,
    funsequence = list("mean"),
    auto_ewr_PU = TRUE
  ...
  spatagg_psx <- multi_aggregate(
    ewr_to_agg,
    causal_edges = causal_ewr,
    aggsequence = list(
      datebits = c(lubridate::ymd('20150101', '20180404', '20200202'))
    ),
    groupers = c("scenario", 'ewr_code', 'ewr_code_main'),
    aggCols = "ewr_achieved",
    saveintermediate = TRUE,
  ...
  expect_snapshot_value(
    spatagg_psx$datebits$date |> unique(),
    style = 'deparse'
  )
})
...
test_that('non-module works', {
  ...
  withr::with_seed(
    ...
    state_inputs <- purrr::map(0:10, \(x) {
      dplyr::mutate(
        state_inputs,
        date = date + x,
        value = value * rnorm(nrow(state_inputs), mean = x, sd = x / 2)
      )
    }) |>
      dplyr::bind_rows()
  )
  ...
  state_inputs <- purrr::imap(letters[1:4], \(x, y) {
    dplyr::mutate(state_inputs, scenario = x, value = value + y)
  }) |>
    dplyr::bind_rows()

  # add a theme-relevant column, each with different values
  state_inputs <- purrr::imap(c("E", "F", "G", "H", "I", "J"), \(x, y) {
    dplyr::mutate(state_inputs, theme1 = x, value = value + y)
  }) |>
    dplyr::bind_rows()

...
test_that("passtidy", {
  mta <- dummyfun(
    groupers = tidyselect::starts_with('ge'),
...
test_that("passbare", {
  mta <- dummyfun(groupers = gear, aggers = mpg)
  expect_equal(names(mta), c('gear', 'mpg'))
})
...
test_that("passtidy", {
  mta <- dummyouter(
    groups = tidyselect::starts_with('ge'),
...
test_that("bare doesn't work higher in the stack", {
  expect_error(mta <- dummyouter(groups = gear, ags = mpg))
})

test_that("passchar", {
  mta <- dummyouter(groups = 'gear', ags = 'mpg')
  expect_equal(names(mta), c('gear', 'mpg'))
})

...
test_that("quant x", {
  ...
  sdl_line_options <- obj_sdl_to_plot |>
    plot_outcomes(
      ...
      facet_col = ".",
      base_list = list(
        base_lev = "base_base", comp_fun = "difference",
...
test_that("maps", {
  ...
  sdl_basin_background_rel <- obj_sdl_to_plot |>
    ...
    plot_outcomes(
      ...
      sceneorder = c("down4_down4", "base_base", "up4_up4"),
      base_list = list(
        base_lev = "base_base", comp_fun = "relative",
        group_cols = c("env_obj", "polyID")
      ),
      zero_adjust = "auto",
...
test_that("heatmaps", {
  ...
  ostp05 <- obj_sdl_to_plot |>
    dplyr::mutate(
      scenario = stringr::str_c(scenario, "minus1"),
  ...
  sdl_contour_base_bin <- ostp |>
    ...
    plot_outcomes(
      ...
      transoutcome = "sqrt",
      base_list = list(
        base_lev = "down4_down4minus1", comp_fun = "difference",
        group_cols = c("env_group", "SWSDLName")
      )
    )
  ...
  sdl_heat_so <- ostp |>
    ...
    plot_outcomes(
      ...
      pal_list = list("scico::turku"),
      sceneorder = c(
        "down4_down4minus1", "down4_down4zero", "down4_down4plus1",
...
test_that("discrete but non-named colors", {
  ...
  sdl_plot_groupblock_dcn <- obj_sdl_to_plot |>
    dplyr::mutate(env_group = dplyr::case_when(
      env_group == "WB" ~ 1,
      env_group == "NV" ~ 2,
      env_group == "EB" ~ 3,
      env_group == "EF" ~ 4,

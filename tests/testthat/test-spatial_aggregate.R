...
test_that("poly to poly works", {
  ...
  namestring <- c(
    'scenario',
    'polyID',
    'spatial_mean_spatial_mean_ewr_achieved',
    'ValleyName',
    'ValleyID',
    'ValleyCode',
    'geometry'
...
test_that("psuedo-spatial works", {
  ...
  expect_equal(
    nrow(spatagg),
    length(unique(spatagg$scenario)) * length(unique(spatagg$polyID))
  )

  # Fail if nothing in common
  expect_error(
    spatagg_no_common <- spatial_aggregate(
      ewr_to_agg_timemean,
...
test_that("multiple functions", {
  ...
  fnlist <- list(mean = ~ mean(., na.rm = TRUE), sd = ~ sd(., na.rm = TRUE))
  expect_warning(
    spatagg_l <- spatial_aggregate(
      ewr_to_agg_timemean,
      to_geo = sdl_units,
      groupers = 'scenario',
      aggCols = 'ewr_achieved',
  ...
  fnlistw <- rlang::quo(list(
    mean = ~ mean(., na.rm = TRUE),
    wm = ~ weighted.mean(., w = wt, na.rm = TRUE)
  ))

  expect_warning(
    spatagg_lw <- spatial_aggregate(
      wtgauge,
      to_geo = sdl_units,
      groupers = 'scenario',
      aggCols = 'ewr_achieved',
  ...
  expect_warning(
    spataggsumdat_lw <- spatial_aggregate(
      dat = wtgauge,
      to_geo = sdl_units,
...
test_that("bare functions", {
  ...
  expect_equal(
    nrow(spatagg),
    length(unique_ewr) * length(unique(spatagg$scenario))
  )
})

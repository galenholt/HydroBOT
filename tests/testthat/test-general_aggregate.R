test_that("characters for everything works", {
  ...
  expect_equal(
    round(aggchar1$agg_mean_disp, 4),
    c(105.1364, 183.3143, 353.1000)
  )

  aggchar2 <- general_aggregate(
    mtcars,
    groupers = c('cyl', 'carb'),
    aggCols = c('disp', 'wt'),
    funlist = c('mean', 'min')
...
test_that("tidyselect for groupers and aggCols works", {
  ...
  aggchar <- general_aggregate(
    mtcars,
    groupers = tidyselect::starts_with('c'),
...
test_that("dots pass", {
  ...
  expect_equal(
    round(aggchar1$agg_mean_disp, 4),
    c(104.4444, 187.2000, 352.5692)
  )
})

test_that("handling data-variables as arguments works with rlang::quo", {
  # \(x) form of anonymous
  aggchar1 <- general_aggregate(
    mtcars,
    groupers = cyl,
    aggCols = disp,
    funlist = rlang::quo(list(mean = \(x) mean(x, na.rm = TRUE), wm = \(x) {
      weighted.mean(x, gear, na.rm = TRUE)
    }))
  )
  ...
  anlist <- rlang::quo(list(mean = \(x) mean(x, na.rm = TRUE), wm = \(x) {
    weighted.mean(x, gear, na.rm = TRUE)
  }))

  aggchare <- general_aggregate(
    mtcars,
    groupers = cyl,
    aggCols = disp,
    funlist = anlist
  ...
  expect_equal(
    round(aggchare$agg_mean_disp, 4),
    c(105.1364, 183.3143, 353.1000)
  )

  # tilde form of anonymous
  aggchart <- general_aggregate(
    mtcars,
    groupers = cyl,
    aggCols = disp,
    funlist = rlang::quo(list(
  ...
  expect_equal(
    round(aggchart$agg_mean_disp, 4),
    c(105.1364, 183.3143, 353.1000)
  )

  # same thing, but specified externally
  anlistt <- rlang::quo(list(
    mean = ~ mean(., na.rm = TRUE),
    wm = ~ weighted.mean(., gear, na.rm = TRUE)
  ))

  aggcharte <- general_aggregate(
    mtcars,
    groupers = cyl,
    aggCols = disp,
    funlist = anlistt
  ...
  expect_equal(
    round(aggcharte$agg_mean_disp, 4),
    c(105.1364, 183.3143, 353.1000)
  )

  # mixed with bare names
  anlistb <- rlang::quo(list(mean = mean, wm = \(x) {
    weighted.mean(x, gear, na.rm = TRUE)
  }))

  aggchareb <- general_aggregate(
    mtcars,
    groupers = cyl,
    aggCols = disp,
    funlist = anlistb
  ...
  wem2 <- function(x, gear) {
    weighted.mean(x, gear, na.rm = TRUE)
  }

  l2 <- rlang::quo(list(mean = mean, wm = wem2))

  aggcharwe2 <- general_aggregate(
    mtcars,
    groupers = cyl,
    aggCols = disp,
    funlist = l2
  ...
  expect_equal(
    round(aggcharwe2$agg_mean_disp, 4),
    c(105.1364, 183.3143, 353.1000)
  )
})

test_that("handling data-variables as arguments works without wrapping with rlang::quo", {
  # \(x) form of anonymous
  aggchar1 <- general_aggregate(
    mtcars,
    groupers = cyl,
    aggCols = disp,
    funlist = list(mean = \(x) mean(x, na.rm = TRUE), wm = \(x) {
      weighted.mean(x, gear, na.rm = TRUE)
    })
  )
  ...
  # same thing, but specified externally
  anlist <- list(mean = \(x) mean(x, na.rm = TRUE), wm = \(x) {
    weighted.mean(x, gear, na.rm = TRUE)
  })
  ...
  # mixed with bare names
  anlistb <- list(mean = mean, wm = \(x) weighted.mean(x, gear, na.rm = TRUE))
  ...
  # mixed with character names- this doesn't work, the quosure prevents the
  # function extraction from quoted names
  anlistc <- list(mean = 'mean', wm = \(x) weighted.mean(x, gear, na.rm = TRUE))
  ...
  # using named, not anonymous functions with data variables
  # This now works even with the second arg buried in the function
  wem <- function(x) {
    weighted.mean(x, gear, na.rm = TRUE)
  }
  ...
  aggcharwe <- general_aggregate(
    ...
    groupers = cyl,
    aggCols = disp,
    funlist = list(mean = mean, wem = wem)
  )

  # it also works when we quo and expose the gear argument
  wem2 <- function(x, gear) {
    weighted.mean(x, gear, na.rm = TRUE)
  }

  l2 <- list(mean = mean, wm = wem2)

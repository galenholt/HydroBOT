test_that("makes a named vector of colors of class 'colors'", {
  ...
  expect_equal(
    simplepal,
    as_colors(stats::setNames(
      c("#E69512FF", "#D3105CFF", "#3B4F8EFF"),
      levnames
    ))
  )
})
...
test_that("reference works", {
  ...
  expect_equal(
    sort(refpal),
    sort(as_colors(stats::setNames(
      c("#E69512FF", "black", "#D3105CFF"),
      levnames
    )))
  )
...
test_that("all as reference works and out of order", {
  ...
  refpal <- make_pal(
    levnames,
    palette = 'calecopal::superbloom3',
    refvals = c('two', 'three', 'one'),
    refcols = c('black', 'forestgreen', 'purple4')
  )
  expect_equal(
    sort(refpal),
    sort(as_colors(stats::setNames(
      c('purple4', 'black', 'forestgreen'),
      levnames
    )))
  )
  expect_equal(class(refpal), 'colors')
...
test_that("informative errors with unknown palettes", {
  ...
  expect_equal(
    err_case$message,
    "Requested palette ggsci::NRC_NPG not present, likely because wrong case or missing letters.
Try `ggsci::nrc_npg`"
  )
  ...
  expect_equal(
    err_missing$message,
    "Requested palette ggsci::nrc not present, likely because wrong case or missing letters.
Try `ggsci::nrc_npg`"
  )
  ...
  expect_equal(
    err_pkg$message,
    "Requested palette nopkg::notapal not available in paletteer"
  )

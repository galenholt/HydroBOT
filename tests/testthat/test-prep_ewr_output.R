...
test_that("year_roll is rolling correctly", {
  ...
  expect_equal(
    sum(is.na(assessed$ewr_achieved)),
    nrow(
      dplyr::distinct(
        dplyr::select(
          assessed,
      ...
      3 -
      6
  ) # the -6 is unclear, seems to arise from duplicate ewrs, so if starts failing in an update, fine to remove
})

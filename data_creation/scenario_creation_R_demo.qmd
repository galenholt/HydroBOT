...
```{r}
catch_demo = rps %>% 
  filter(SWWRPANAME %in% c('Macquarie-Castlereagh', 'Lachlan', 'Namoi'))
```
...
demo_levs <- select(demo_levs, -DATETIME) %>% 
...
```{r}
# gaugegeo <- gaugegeo %>% 
#   rename(gauge = `gauge number`)
demo_geo = gaugegeo %>% filter(gauge %in% py$demo_ids)
```
...
```{r}
base <- demo_levs %>% 
  select(Date, VALUE, SITEID) %>% 
  pivot_wider(id_cols = Date, names_from = SITEID, values_from = VALUE)
...
```{r}
mac = rps %>% 
  filter(SWWRPANAME %in% c('Macquarie-Castlereagh'))
lach = rps %>% 
  filter(SWWRPANAME %in% 'Lachlan')
...
```{r}
macsub = macgauge %>% 
  filter(gauge %in% c('421004', '421001', '421011'))
lachsub = lachgauge %>% 
  filter(gauge %in% c('412005', '412002', '412038'))

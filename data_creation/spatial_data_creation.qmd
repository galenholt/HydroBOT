...
Set the directory. This got convoluted with the Rproject nested inside quarto, but I think this largely keeps the R self-contained while still allowing Quarto to render. I think I will eventually be able to use `library(werptoolkitr)`, but quarto is grumpy about it, so skip for now.
...
if (!dir.exists(datarawdir)) {
  dir.create(datarawdir, recursive = TRUE)
}
if (!dir.exists(dataoutdir)) {
  dir.create(dataoutdir, recursive = TRUE)
}
if (!dir.exists(dataextdir)) {
  dir.create(dataextdir, recursive = TRUE)
}
```
...
```{r}
basin <- read_sf(file.path(
  datarawdir, "downloaded",
  "mdb_boundary",
  "mdb_boundary.shp"
))
```
...
```{r}
cewo_valleys <- read_sf(dsn = file.path(
  datarawdir, "downloaded",
  "cewo_valleys",
...
```{r}
print(vertcount(basin_rivers))
```

```{r}
basin_rivers <- basin_rivers |> 
  rmapshaper::ms_simplify(keep = 0.01)
...
ggplot() +
  ...
  geom_sf(data = basin_rivers, aes(color = log(UpstrGeoLn))) +
...
if (REBUILD_DATA) {
  ...
  st_write(basin, 
           file.path(dataextdir, 'basin.shp'), 
           append = FALSE)
  st_write(resource_plan_areas, 
           file.path(dataextdir, 'resource_plan.shp'), 
           append = FALSE)
  st_write(sdl_units, 
           file.path(dataextdir, 'sdl_units.shp'), 
           append = FALSE)
  st_write(bom_basin_gauges, 
           file.path(dataextdir, 'bom_gauges.shp'), 
           append = FALSE)
  st_write(cewo_valleys, 
           file.path(dataextdir, 'cewo_valleys.shp'), 
           append = FALSE)
    st_write(planning_units, 
           file.path(dataextdir, 'planning_units.shp'), 
           append = FALSE)
    st_write(basin_rivers, 
           file.path(dataextdir, 'basin_rivers.shp'), 
           append = FALSE)

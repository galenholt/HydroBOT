...
Get a list of what exists, don't create if it does.

```{r}
datarawdir <- 'data-raw'
# existing_dirs <- list.files(datarawdir)
```
...
if (REBUILD_DATA) {
  st_write(basin, 
           file.path('data', 'basin.shp'), 
           append = FALSE)
  st_write(rpa_simple, 
           file.path('data', 'resource_plan.shp'), 
           append = FALSE)
  st_write(sdls_simple, 
           file.path('data', 'sdl_units.shp'), 
           append = FALSE)
  st_write(bombasin, 
           file.path('data', 'bom_gauges.shp'), 
           append = FALSE)
  st_write(cewo_Valleys, 
           file.path('data', 'cewo_valleys.shp'), 
           append = FALSE)
}

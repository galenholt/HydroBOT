...
summary_ewr_output <- get_ewr_output(ewrpath, type = 'summary')
  ...
  usethis::use_data(summary_ewr_output, overwrite = REBUILD_DATA)
...
```{r}
  usethis::use_data(agg_theme_space, overwrite = REBUILD_DATA)
```
...
```{r}
  usethis::use_data(agg_theme_space_colsequence, overwrite = REBUILD_DATA)
```

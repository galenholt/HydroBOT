...
ced[1:2] <- ced[1:2] |>
  purrr::map(\(x) {
    x$planning_unit_name[
      x$planning_unit_name == 'Border rivers'
    ] <- 'Border Rivers and Moonie Long-term watering plan'
    return(x)
  })
...
parsh <- readr::read_csv(
  '../EWR_tool/py_ewr/parameter_metadata/parameter_sheet.csv',
  guess_max = 5000
)
updatenest <- readr::read_csv(
  '../EWR_tool/unit_testing_files/MURRAY_MDBA_update_nest.csv',
  guess_max = 5000
) |>
  dplyr::mutate(Gauge = as.character(Gauge))

updateqld <- readr::read_csv(
  '../EWR_tool/unit_testing_files/qld_parameter_sheet.csv',
  guess_max = 5000
) |>
  dplyr::mutate(Gauge = as.character(Gauge))

updatesa <- readr::read_csv(
  '../EWR_tool/unit_testing_files/sa_parameter_sheet.csv',
  guess_max = 5000
) |>
  dplyr::mutate(Gauge = as.character(Gauge))

updatevic <- readr::read_csv(
  '../EWR_tool/unit_testing_files/vic_parameter_sheet.csv',
  guess_max = 5000
) |>
  dplyr::mutate(Gauge = as.character(Gauge))
...
retc2 |>
  dplyr::filter(
    planning_unit_name ==
      'Warrego Paroo Bulloo and Nebine Long-term watering plan'
  )
...
retc |>
  ...
  dplyr::arrange(planning_unit_name, gauge, ewr_code, ewr_code_timing) |>
  View()
...
retc |>
  ...
  dplyr::select(-gauge) |>
  dplyr::distinct() |>
  dplyr::arrange(planning_unit_name, ewr_code, ewr_code_timing) |>
  View()
...
e2o_test |>
  ...
  dplyr::arrange(planning_unit_name, ewr_code, ewr_code_timing, env_obj) |>
  View()

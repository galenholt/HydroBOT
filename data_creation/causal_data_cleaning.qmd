...
# if (REBUILD_DATA) {saver <- 'r'} else {saver <- FALSE}

# actually, just save the list at the end. `saver` is for individual functions.
saver <- FALSE
```
...
```{r}
datout <- file.path('data', 'causal_networks')
if (!dir.exists(datout)) {dir.create(datout, recursive = TRUE)}
```

And the directory with the input data

```{r}
datadir <- file.path('data-raw', 'causal_networks')
```
...
```{r}
# Get ewr defs from MDBA
ewrs <- file.path(datadir, 'OLD_NSWEWR_ECOLOGICAL_OBJECTIVES.csv')
...
objtable <- file.path(datadir, "tbl10a_WatRequirements5.4.csv")
```
...
```{r}
tarpath <- file.path(datadir, "Env_objectives_Target_species_nodes.csv")

qcs <- c(file.path(datadir, "Target_species_per_PU_Murray.csv"),
         file.path(datadir, "PUs.csv"))

timepath <- file.path(datadir, "Env_objectives_5_10_20_year_targets_nodes Murray Lower Darling.csv")
```
...
ewr2obj %>% select(gauge, ewr_code, env_obj, PlanningUnitID, everything())
```
...
obj2target %>% select(PlanningUnitID, env_obj, Specific_goal, Objective, Target, everything())
```
...
```{r}
yrtarget %>% select(env_obj, Target, Objective, starts_with('target'), everything())
```

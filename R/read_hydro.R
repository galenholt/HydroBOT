...
read_hydro_nc <- function(hydropath, scenariofilter, long, gaugemap) {
  ...
  # ncdf4 is the main ncdf package, but metR's wrapper is WAY nicer to use, and
  # I don't have to reinvent the wheel to retain date formats and make tidy.
  # nco <- ncdf4::nc_open(fp)
  # metR::GlanceNetCDF(fp)

  if (!inherits(gaugemap, "data.frame") && gaugemap == "iqqm") {
    gaugemap <- get_iqqm_gauges() |>
      dplyr::mutate(node = as.integer(.data$iqqm_node)) |>
      dplyr::select(-"iqqm_node")
  }

  # we need to purrr over hydro_paths
  readscenes <- function(x, idx) {
    # this is fairly specific to Ash's naming conventions. If that changes,
    # we'll need to be more general

...
assess_ewr_flex <- function(annualdf, year_roll, k_freq, k_inter, combo) {
  ...
  annualdf <- annualdf |>
    ...
    dplyr::mutate(
      ...
      frequency_achieved = adjusted_logistic(
        x = .data$frequency_occurred,
        threshold = .data$target_frequency,
        k = k_freq,
        thresh_is = 'min'
      ),
      interevent_achieved = adjusted_logistic(
        x = .data$rolling_max_inter_event,
      ...
      ewr_achieved = combo[[1]](
        .data$frequency_achieved,
        .data$interevent_achieved
      )
    ) |>
    # dplyr::ungroup() |>

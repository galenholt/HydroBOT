...
#' @param edge_pal a named list of color palettes or color values for the edges.
#'   Currently only supports paletteer palettes, vectors of colors, or single
#'   colors. See \code{\link{causal_colors_general}}
...
#' @param node_pal a named list of color palettes or color values for the nodes.
#'   Currently only supports paletteer palettes, vectors of colors, or single
#'   colors. See \code{\link{causal_colors_general}}
...
#'   TRUE`
#' @param savename character, filename for the output plots. .pdf and .png are
#'   automatically appended.
#' @param edge_pal_direction either 1 (default) or -1 (reversed) direction of
#'   the palettes
#' @param node_pal_direction either 1 (default) or -1 (reversed) direction of
#'   the palettes
#'
...
make_causal_plot <- function(nodes, edges,
  ...
  # IF WE"RE GOING TO WRAP NAMES IT NEEDS TO HAPPEN BY HERE or the rest of this
  # doesn't match
  if (wrap_names) {
    edges <- edges |>
      dplyr::mutate(
        from = stringr::str_wrap(.data$from, 40),
        to = stringr::str_wrap(.data$to, 40)
      )
    nodes <- nodes |>
      dplyr::mutate(Name = stringr::str_wrap(.data$Name, 40))
  }
  ...
  if (drop_unused_nodes) {
    nodes <- nodes |>
      dplyr::filter(.data$Name %in% edges$from | .data$Name %in% edges$to)
  }
  ...
  # from_to_map is just an argument that says to label the fromto, it's not an
  # env or data variable
  # The attributes (color etc) are not called out here at all, and depend on
  # having the right column names in the dfs

  # Cryptic errors here related to `c.c` and extra rows seem to be caused by
  # having rows that are duplicated over the Name and NodeType columns (even if
  # there are differences elsewhere)
  causalnetwork <-

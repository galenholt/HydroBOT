...
handle_palettes <- function(
  ...
  if (color_type == 'grouped') {
    ...
    if (aes_type == 'fill') {
      ggobj <- ggobj +
        ggplot2::scale_fill_identity(guide = 'legend', labels = \(x) {
          labfind(x, ggobj$data)
        })
    }
    if (aes_type == 'color') {
      ggobj <- ggobj +
        ggplot2::scale_color_identity(guide = 'legend', labels = \(x) {
          labfind(x, ggobj$data)
        })
    }
  }
  ...
  if (!is.null(lims) & length(lims) %in% c(1, 3)) {
    if (length(lims) == 1) {
      if (!rlang::is_function(get(trans))) {
        rlang::abort(glue::glue(
          "`transoutcome` ({trans}) is not a named function and so cannot be applied to find y-scales.
    ...
    if (middown != middup) {
      rlang::inform(glue::glue(
        "Limits {lims} not symmetrical about the midpoint.

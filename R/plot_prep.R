...
plot_data_prep <- function(data, outcome_col,
  ...
  if (is.null(base_list) & zero_adjust != 0) {
    if (!("relative" %in% base_list$comp_fun)) {
      rlang::warn(glue::glue("`comp_fun` is {base_list$comp_fun}, but you're adjusting the data by {zero_adjust}.
  ...
  if (grepl("auto", zero_adjust)) {
    zero_adjust <- min(abs(data[[outcome_col]])[data[[outcome_col]] != 0],
      na.rm = TRUE
    ) * 0.1
  }
  ...
  if (!is.null(base_list)) {
    ...
    data <- data |>
      baseline_compare(
        compare_col = "scenario", base_lev = base_list$base_lev,
...
plot_style_prep <- function(prepped, colorset, colorgroups, pal_list, pal_direction,
  ...
  prepped <- utils::modifyList(prepped, list(

...
plot_data_prep <- function(
  ...
  if (!is.null(sceneorder) && "scenario" %in% names(data)) {
    if (inherits(sceneorder, "factor")) {
      sceneorder <- levels(sceneorder)
    }
    data <- data |>
      dplyr::mutate(scenario = forcats::fct_relevel(.data$scenario, sceneorder))
  }

  # if outcome_col is NULL, as happens with maps sometimes, or non-numeric, just
  # skip this entirely there's no y data to adjust mathematically
  ...
  if (is.null(base_list) & zero_adjust != 0) {
    if (!("relative" %in% base_list$comp_fun)) {
      rlang::warn(glue::glue(
        "`comp_fun` is {base_list$comp_fun}, but you're adjusting the data by {zero_adjust}.
                             Do you really want to do that?
                             It may be appropriate if you're log-transforming or dividing, but be careful.\n"
      ))
    }
  }
  ...
  if (grepl("auto", zero_adjust)) {
    zero_adjust <- min(
      abs(data[[outcome_col]])[data[[outcome_col]] != 0],
      na.rm = TRUE
    ) *
      0.1
  }
  ...
  if (!is.null(base_list)) {
    ...
    data <- data |>
      baseline_compare(
        ...
        base_lev = base_list$base_lev,
        values_col = outcome_col,
        comp_fun = base_list$comp_fun,
    ...
    if (is.character(base_list$comp_fun)) {
      ...
      rlang::abort(
        "getting the name of comp_fun isn't supported,
    ...
    names(data)[
      names(data) == paste0("comp_fun_", outcome_col)
    ] <- base_col_name
    ...
    ylab_append <- paste0(
      ...
      comp_fun_name,
      " to ",
      as.character(base_list$base_lev)
    )
  }

  # That can introduce some infs and nans, often from division by zero in the
  # relativiser
  new_nan_inf <- sum(is.nan(data[[outcome_col]])) +
    sum(is.infinite(data[[outcome_col]]))
  old_nan_inf <- sum(is.nan(data[[outcome_col]])) +
    sum(is.infinite(data[[outcome_col]]))
  if (new_nan_inf > old_nan_inf) {
    rlang::warn(glue::glue(
      "NaN and Inf introduced in `plot_data_prep`, likely due to division by zero. {new_nan_inf - old_nan_inf} values were lost."
    ))
  }

  # This names the data the same thing as it was interactively, but fails in the
  # function. Just call it data for consistency
  return(tibble::lst(data, outcome_col, ylab_append))
}
...
plot_style_prep <- function(
  ...
  if (
    (grepl("paletteer", color_type) | grepl("grouped", color_type)) &
      (length(pal_list) != length(pal_direction))
  ) {
    rlang::abort(glue::glue(
      "pal_list is trying to choose paletteer colors from a list {length(pal_list)} long,
  ...
  if (!is.null(colorset)) {
    if (
      is.numeric(dplyr::pull(sf::st_drop_geometry(prepped$data[,
        colorset,
        drop = FALSE
      ])))
    ) {
  ...
  if (color_type == "grouped") {
    ...
    prepped$data <- prepped$data |>
      dplyr::mutate(color = forcats::fct_inorder(.data$color)) # Works, but explictly making it line up with colordef might be better?
  ...
  if (is.null(point_group)) {
    ...
    prepped$data <- prepped$data |>
      dplyr::mutate(
        pointgroup = interaction(.data[[colorset]], .data[[point_group]])
      )
  }

  prepped <- utils::modifyList(
    prepped,
    list(

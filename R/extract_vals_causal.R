...
#' @export
#'
extract_vals_causal <- function(
  agglist,
  whichaggs,
  valcol,
  targetlevels = names(agglist)
) {
  ...
  ) %do%
    {
      ...
      if (length(thesenodes) > 1) {
        rlang::abort(glue::glue(
          "Expect each node level to have one value,
      ...
      numaggs <- max(grep(
        '[1-9]',
        names(simpledf)[grepl('aggfun', names(simpledf))]
      ))

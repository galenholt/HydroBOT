...
#' @param pal_list list of palettes, or color name or color object that maps to
#'   `colorgroups`, `colorset`, or `scenario`. Typically a list of names of
#'   `palletteer` palettes of the same length as unique values in `colorgroups`
#' @param pal_direction vector of length pal_list, either 1 (default) or -1
#'   (reversed) direction of the palettes
#' @param colorgroups character column name of column defining the groups that
#'   define different palettes to use
#' @param colorset  character column name of the values to assign colors from
#'   those palettes
...
grouped_colors <- function(df, pal_list,
  ...
  if (is.numeric(dfcols$colordef)) {
    ...
    # simple case with two limits, or complex with 3 (middle is centerpoint for
    # diverging)
  ...
  # *very* close to `make_pal`, and could be combined, but the issue is the
  # rowwise loop. Just need a bit more thinking there

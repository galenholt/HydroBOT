...
baseline_compare <- function(
  ...
  if (length(values_col) > 1) {
    values_col <- values_to
  }
  ...
  if (is.character(comp_fun)) {
    if (length(comp_fun) > 1) {
      rlang::abort('baselining with more than one function not supported.')
    }
    comp_fun <- functionlister(comp_fun)
    ...
    val_df <- val_df |>
      dplyr::mutate(dplyr::across(
        {{ valcols }},
        \(x) {
          comp_fun[[1]](x, y = .data[[stringr::str_c('ref_', valcols)]], ...)
        },
        .names = nameparser
      ))
  }
...
create_base <- function(
  ...
  if (length(valcols) > 1) {
    rlang::warn(
      "data with multiple values columns. Making long and will return long with column names in 'name' column"
    )
    val_df <- val_df |>
      tidyr::pivot_longer(
        {{ valcols }},
  ...
  if (is.null(group_cols)) {
    ...
    ndups <- val_df |>
      dplyr::group_by(dplyr::across(tidyselect::any_of(c(
        compcols,
        'geometry'
      )))) |>
    ...
    if (ndups > 1) {
      ...
      nonnumnames <- nonnumnames[
        !(nonnumnames %in% c(compcols, valcols, 'geometry'))
      ]
      rlang::warn(glue::glue(
        "`group_cols` argument not passed, but multiple
  ...
  if (
    ...
    refdf <- tibble::tibble(name = orig_valcols, refcol = base_lev) |>
      ...
      dplyr::rename_with(.fn = ~names_to, .cols = "name") |>
      sf::st_drop_geometry()

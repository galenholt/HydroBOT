...
causal_colors_general <- function(df, pal_list, 
  ...
  cnames <- paletteer::palettes_c_names %>% 
    mutate(formatted = str_c(package, palette, sep = '::')) %>% 
    select(formatted) %>% pull()
  ...
  # Make a df without dups and left_join
  dfcols <- dfcolor %>%
    group_by(across(any_of(colorgroups))) %>% 
    distinct(colordef) %>% 
    mutate(palname = ifelse(is.null(colorgroups), 
                            unlist(pal_list),
  ...
  # should be doable with case_when but it won't run in a mutate. Loop over
  ...
  dfcols <- bind_cols(dfcols, color = as.character(colmap)) %>% 
    select(-c(palname, pallength, palindex))
  ...
  suppressMessages(dfcolor <- left_join(dfcolor, dfcols))
...
make_colorcol <- function(df, colorset) {
  # These are all mutates to make sure we can re-join the outcome
  ...
  if (is.null(colorset)) {
    dfgroup <- df %>% 
      mutate(colordef = .data[[namecol]])
  } else if (colorset == 'werp') {
    # Set up a default set of groupings for werp
    dfgroup <- df %>% 
      mutate(colordef = case_when(.data[[typecol]] == 'ewr_code' ~ str_extract(.data[[namecol]], '^[A-Z]+'),
                                  .data[[typecol]] == 'env_obj' ~ str_extract(.data[[namecol]], '^[A-Z]+'),
                                  .data[[typecol]] == 'Specific_goal' ~ .data[[namecol]],
                                  .data[[typecol]] == 'Target' ~ .data[[namecol]],
                                  str_detect(.data[[typecol]], 'target_') ~ str_extract(.data[[typecol]], '[0-9]_year')))
    
  } else {
    # This is silly to rename the col, but safer
    dfgroup <- df %>% 
      mutate(across(all_of(colorset), identity, .names = 'colordef'))
  }
...
tweak_node_color <- function(df) {
    df <- df %>% 
      rename(fillcolor = color) %>% 
      # create the fontcolor
      mutate(fontcolor = fontcol(fillcolor))
}
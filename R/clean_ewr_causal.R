...
clean_ewr_237 <- function(ewrnet) {
  ...
  e2o <- ewrnet$ewr2obj |>
    ...
    dplyr::mutate(
      ...
      Target = ifelse(
        grepl('Ecosystem functions', .data$Target),
        'Ecosystem function',
        .data$Target
      ),
      Target = ifelse(
        grepl('Q-NF', .data$env_obj) &
          is.na(.data$Target),
        'Native fish',
        .data$Target
      )
  ...
  e2o <- e2o |>
    dplyr::mutate(
      Target_short = dplyr::case_when(
        Target == "Ecosystem function" ~ "EF",
  ...
  Gol_Reach5_NF4 <- e2o |>
    dplyr::filter(
      env_obj == "NF4" &
        planning_unit_name == "Reach 5: Loch Garry to the Murray River" &
  ...
  e2o <- e2o |>
    dplyr::filter(
      !env_obj %in% c("NF1", "NF2", "NF3", "NF4") |
        env_obj %in% c("NF1", "NF2", "NF3", "NF4") & Target == "Native fish"
    ) #VERY NEAT!!
  ...
  o2t <- ewrnet$obj2target |>
    dplyr::mutate(
      ...
      Target = ifelse(
        grepl('Priority ecosystem function', .data$Target),
        'Ecosystem function',
        .data$Target
      ),
      Target = stringr::str_to_sentence(.data$Target),
      Target = ifelse(
        grepl('Waterbird', .data$Target),
        'Waterbirds',
        .data$Target
      ),
      Target = ifelse(
  ...
  o2yt <- ewrnet$obj2yrtarget |>
    dplyr::mutate(
      ...
      Target = ifelse(
        grepl('Q-NF', .data$env_obj) & is.na(.data$Target),
        'Native fish',
        .data$Target
      )
    ) |>
...
check_causal_issues <- function(causal_list) {
  ...
  if (!inherits(clean_causal, 'data.frame')) {
    ...
    purrr::imap(unique_causals, \(x, i) {
      rlang::inform(glue::glue(
        'Unique values of {i} are {paste(x, collapse = ", ")}'
      ))
    })
  }

...
add_polyID <- function(geosf, failduplicate = TRUE) {
  ...
  if (length(badgeom) > 0 & failduplicate) {
    rlang::abort(c(
      "Empty geometries while geohashing. Fix to proceed.",
      glue::glue("{length(badgeom)} rows have empty geometries.")
    ))
  }
  if (length(badgeom) > 0 & !failduplicate) {
    rlang::warn(c(
      "Empty geometries while geohashing replaced with NA.",
      glue::glue("{length(badgeom)} rows have empty geometries."),
  ...
  geosf$polyID <- NA
  geosf$polyID[goodgeom] <- lwgeom::st_geohash(
    geosf$geometry[goodgeom],
    precision = 11
  )

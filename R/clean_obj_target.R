...
clean_obj_target <- function(
  ...
  obj2target <- obj2target |>
    ...
    dplyr::filter(
      !(.data$LTWPShortName == "Macquarie-Castlereagh" &
        .data$Macquarie_Castlereagh == 0)
    ) |>
    dplyr::filter(
      !(.data$LTWPShortName == "Murrumbidgee" & .data$Murrumbidgee == 0)
    ) |>
    dplyr::select(
      -c(
        .data$Murray_Lower_Darling,
        .data$Macquarie_Castlereagh,
        .data$Murrumbidgee
      )
    )
  ...
  qc_fix <- readr::read_csv(
    qcfiles[1],
    col_types = readr::cols(),
    col_select = -1
  ) |>
    dplyr::rename(
      Specific_goal = .data$Target.species,
      env_obj = .data$Env_obj
    ) |>
    dplyr::select(-.data$NodeType)

  PUs_names <- readr::read_csv(
    qcfiles[2],
    col_types = readr::cols(),
    col_select = -1
  )

  qc_fix <- dplyr::left_join(
    qc_fix,
    PUs_names,
    by = "PU",
    relationship = 'many-to-many'
  ...
  obj2target <- obj2target |>
    dplyr::select(
      tidyselect::any_of(c('PlanningUnitID', 'planning_unit_name')),
      .data$LTWPShortName,
      .data$env_obj,
      .data$Specific_goal,
      .data$Objective,

...
To run, this needs a Python environment containing `py_ewr` (currently `r get_ewr_version()`). The package will manage that for you if you just start using it- on first use, the package checks the environment and either uses an existing python environment or builds one with that dependency when the package is loaded.

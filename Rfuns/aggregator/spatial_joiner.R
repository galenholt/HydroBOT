...
#'   for speed, and expected to add back in later with a `left_join` to `to_geo`
...
spatial_joiner <- function(from_geo, to_geo, whichcrs) {
  ...
  from_poly <- from_geo %>% 
    select(polyID_f = polyID) %>% 
    group_by(polyID_f) %>% 
    slice(1) %>% # usual use of distinct() checks the polys factorially. slice just indexes.
    ungroup() %>% 
  ...
  to_poly <- to_geo  %>% 
    select(polyID_t = polyID) %>% 
    group_by(polyID_t) %>% 
    slice(1) %>% # usual use of distinct() checks the polys factorially. slice just indexes.
    ungroup()
  
  from_data <- st_drop_geometry(from_geo) %>% 
    rename(polyID_f = polyID)
  to_data <- st_drop_geometry(to_geo) %>% 
    rename(polyID_t = polyID)
  ...
  if (all(st_is(from_geo, 'POINT'))) {
    ...
    fromto_data <- from_data %>% 
      left_join(fromto_pair, by = 'polyID_f') %>% 
      left_join(to_data, by = 'polyID_t') %>% 
      select(everything(), polyID = polyID_t, -polyID_f)

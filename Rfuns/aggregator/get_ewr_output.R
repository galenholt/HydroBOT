...
get_ewr_output <- function(dir, type,
  ...
  relevantfiles <- gaugefiles[str_which(gaugefiles, pattern = type)]
  
  # cut to requested gauges or scenarios
  if (!is.null(gaugefilter)) {
    relevantfiles <- relevantfiles[str_which(relevantfiles, 
                                             pattern = str_flatten(gaugefilter, collapse = '|'))]
  }
  
  if (!is.null(scenariofilter)) {
    relevantfiles <- relevantfiles[str_which(relevantfiles, 
                                             pattern = str_flatten(scenariofilter, collapse = '|'))]
  }
...
cleanSummary <- function(summarydf) {
  ...
  summarydf <- summarydf %>%
    mutate(ewr_achieved = target_frequency <= frequency) 
}
...
cleanYrEWRs <- function(ewrdf) {
  ...
  codecols <- str_which(names(ewrdf), '([0-9])|([A-Z])')
  daysbetweencols <- str_which(names(ewrdf), 'daysBetweenEvents')
  refcols <- which(!(names(ewrdf) %in% names(ewrdf)[codecols]))
  
  # Split and stack the columns
  gaugelong <- ewrdf %>% 
    select(-all_of(daysbetweencols)) %>%
    # The cols would be nice to be a more general spec.
    pivot_longer(cols = matches('([0-9])|([A-Z])', ignore.case = FALSE), # matches('([0-9])|([A-Z])')
                 names_to = 'EWR_stat', 
                 values_to = 'value') %>%
    separate(EWR_stat, into = c('ewr_code', 'stat'), sep = '(_)(?!.*_)')
  
  # Do that again for the daysBetweenEvents
  gaugelongdays <- ewrdf %>% 
    select(all_of(c(refcols, daysbetweencols))) %>%
    # The cols would be nice to be a more general spec.
    pivot_longer(cols = matches('([0-9])|([A-Z])', ignore.case = FALSE), # matches('([0-9])|([A-Z])')
                 names_to = 'EWR_stat', 
                 values_to = 'value') %>%
  ...
  allgaugestats <- left_join(gaugestats, gaugestatsdays, by = c(names(ewrdf)[refcols], 'ewr_code'))

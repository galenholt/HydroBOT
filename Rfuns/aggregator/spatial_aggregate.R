...
spatial_aggregate <- function(dat, to_geo, groupers,
  ...
  if (keepAllPolys) {
    ...
    if (nrow(unusedPolys) > 0) {
      ...
      allgroups <- fromto_pair %>% distinct(across({{groupers}}))
      # combine
  ...
  # could just use bare aggCols, but it throws warnings. Could use ends_with too, as in theme.
  ...
  # step.
  aggPoly <- left_join(agged, to_geo, by = 'polyID') %>%
    st_as_sf()

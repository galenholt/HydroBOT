...
make_nodes <- function(edgedf, groupers = NULL, typeorder = 'werp') {
  ...
  if (is.character(typeorder)) {
    nodetib <- tibble(NodeType = typeorder) %>% 
      mutate(nodeorder = row_number())
  ...
  fromnodes <- edgedf %>% 
    select(any_of(groupers), Name = from, NodeType = fromtype) 
  
  tonodes <- edgedf %>% 
    mutate(nodeorder = edgeorder + 1) %>% 
    select(any_of(groupers), Name = to, NodeType = totype)
  
  allnodes <- bind_rows(fromnodes, tonodes) %>%
    group_by(Name, NodeType) %>%
    ungroup() %>% 
    group_by(across(any_of(groupers))) %>% 
    distinct() %>% 
    ungroup()
  ...
  realisedorders <- nodetib %>% 
    ...
    mutate(shiftorders = (nodeorder - min(nodeorder)) + 1,
           ordersteps = cumsum(c(0, (diff(shiftorders)-1))),
           neworders = shiftorders - ordersteps) %>% 
    select(NodeType, nodeorder = neworders)
  
  # Join o get the orders set
  allnodes <- left_join(allnodes, realisedorders, by = 'NodeType')

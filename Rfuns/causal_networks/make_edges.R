...
make_edges <- function(dflist,
  ...
  alledges <- foreach (p = fromtos, i = iterators::icount(), .combine = bind_rows) %do% {
    ...
    thisdf <- thisdf %>% 
      ...
      rename(from = p[1], to = p[2]) %>% 
      select(any_of(c('gauge', 'PlanningUnitID', extrasave, 'from', 'to', 'color'))) %>% 
      mutate(fromtype = p[1],
             totype = p[2],
...
filtergroups <- function(edgedf, 
  ...
  if (is.null(fromfilter)) {
    fromfilter <- edgedf %>% select(all_of(fromcol)) %>% distinct() %>% pull()
  }
  
  if (is.null(tofilter)) {
    tofilter <- edgedf %>% select(all_of(tocol)) %>% distinct() %>% pull()
  }

...
clean_obj_target <- function(ewrobjs,
  ...
  objpu <- ewrobjs %>% 
    distinct(PlanningUnitID, LTWPShortName, env_obj)
  ...
  targets <- read_csv(targetpath, col_types = cols())
  ...
  targets <- targets %>%
    mutate(Objective = str_remove_all(Objective, '\'')) %>% 
    rename(env_obj = Env_obj) %>% 
    select(-NodeType)
  
  
  # Expands the objective to target mapping to all relevant planning units
  obj2target <- full_join(objpu, targets,  by = "env_obj")
  ...
  obj2target <- obj2target %>% 
    ...
    select(-c(Murray_Lower_Darling,	Macquarie_Castlereagh,	Murrumbidgee))
  ...
  suppressWarnings(qc_fix <- read_csv(qcfiles[1], col_types = cols()) %>% 
    rename(Specific_goal = Target.species,
           env_obj = Env_obj) %>% 
    select(-NodeType))
  
  PUs_names <- suppressWarnings(read_csv(qcfiles[2], col_types = cols())) #the names didnt match the way i was doing it before
  PUs_names$X1 <- NULL
  qc_fix$X1 <- NULL
  qc_fix <- left_join(qc_fix, PUs_names, by = "PU")
  ...
  pu2ltwp <- ewrobjs %>% 
    distinct(PlanningUnitID, LTWPShortName)
  
  qc_fix <- left_join(qc_fix, pu2ltwp, by = 'LTWPShortName')
  
  qc_fix <- qc_fix %>%
    filter(link != 0) %>% #watch out for the 2s = Renee changes
    select(-c(link, PU, PlanningUnitName)) # get rid of extra cols not in the main data
  
  obj2target <- bind_rows(obj2target, qc_fix)
  
  # cleanup column ordering
  obj2target <- obj2target %>%
    select(PlanningUnitID, LTWPShortName, 
           env_obj,
           Specific_goal, Objective, Target)
  
  # final cleanup of weird characters and rename to standard

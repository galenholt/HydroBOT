...
Rebuilding data is done with params. To rebuild, at the terminal run `quarto render data_creation/controller_ewr_testrun_R.qmd -P REBUILD_DATA:TRUE` or, to rebuild *everything*, run `quarto render -P REBUILD_DATA:TRUE` (or without the parameters if we want to re-render but not rebuild.)
...
scenario_dir = file.path("inst/extdata/testsmall/hydrographs")

output_dir =file.path("inst/extdata/testsmall")
```
...
outpath = make_output_dir_R(output_dir, sceneinfodict)

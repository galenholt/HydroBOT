...
Rebuilding data is done with params. To rebuild, at the terminal in WERP_toolkit run `quarto render data_creation/scenario_creation_R_demo.qmd -P REBUILD_DATA:TRUE`. or, to rebuild *everything*, run `quarto render -P REBUILD_DATA:TRUE` (or without the parameters if we want to re-render but not rebuild.)
...
# a minimal set
for (x in scenenames) {
      scenedirS = file.path(dataextdir, 'testsmall', x)
...
if (REBUILD_DATA) {
  start15 = lubridate::ymd(20150101)
    ...
    base %>% 
      ...
      readr::write_csv(file.path(dataextdir, 'testsmall', 'base', 'base.csv'))
    
    down4 %>% 
      dplyr::filter(Date > start15) %>% 
      dplyr::select(Date, tidyselect::all_of(minigauges)) %>%  
      readr::write_csv(file.path(dataextdir, 'testsmall', 'down4', 'down4.csv'))
    
    up4 %>% 
      dplyr::filter(Date > start15) %>% 
      dplyr::select(Date, tidyselect::all_of(minigauges)) %>%  
      readr::write_csv(file.path(dataextdir, 'testsmall', 'up4', 'up4.csv'))
}
